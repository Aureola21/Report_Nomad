...
The robot navigation scenario illustrates how a diffusion model generates action trajectories toward a goal. The green and red lines represent different sampled action sequences, indicating various stages in the diffusion process—ranging from noisy to refined predictions. The final selected trajectory, shown in pink, moves the robot from its current position toward the predicted goal. The spread of sampled paths highlights the model’s uncertainty in determining the optimal route. For visual reference, see the figures in the\hyperref[app:methods]{Appendix X}\\ appendix.\subsection*{Train action samples:}
...
\begin{itemize}
    ...
    \item \hyperref[app:methods]{\textbf{Appendix X:}} Experiments and Results
